...
export default function Settings() {
  ...
  const [ariaRoles, setAriaRoles] = useState<string[]>(['SUPER_ADMIN', 'ADMIN', 'COMPLIANCE', 'SUPPORT']);
  const [success, setSuccess] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [initError, setInitError] = useState<string | null>(null);

  const canEdit = hasPermission ? hasPermission('settings:update') : true;
  ...
  const loadSettings = async () => {
    ...
    try { 
      ...
      // Usar settings por defecto para evitar página en blanco
      setSettings([
        { id: '1', key: 'fci_annual_rate', value: '22.08', category: 'rates', description: 'Tasa FCI Anual', data_type: 'number', is_sensitive: false, updated_at: new Date().toISOString() },
        { id: '2', key: 'default_penalty_rate', value: '3', category: 'rates', description: 'Penalidad Mora', data_type: 'number', is_sensitive: false, updated_at: new Date().toISOString() },
        { id: '3', key: 'min_investment', value: '10000', category: 'limits', description: 'Inversión Mínima', data_type: 'number', is_sensitive: false, updated_at: new Date().toISOString() },
        { id: '4', key: 'max_financing_percent', value: '15', category: 'limits', description: 'Financiamiento Máx %', data_type: 'number', is_sensitive: false, updated_at: new Date().toISOString() },
      ]);
      if (err.response?.status !== 404) {
        setInitError('No se pudieron cargar las configuraciones del servidor. Mostrando valores por defecto.');
      }
    }
  ...
  const loadAriaConfig = async () => {
    try {
      ...
      if (res.data?.data) {
        setAriaEnabled(res.data.data.enabled ?? true);
        setAriaRoles(res.data.data.allowedRoles || ['SUPER_ADMIN', 'ADMIN']);
      }
    } catch (err) { console.error('Error loading aria config:', err); }
  };

  const loadHistory = async () => {
    try { 
      const res = await settingsApi.getHistory({ limit: 50 }); 
      setHistory(res.data?.data?.history || []); 
      setHistoryOpen(true); 
    }
    catch (err) { console.error('Error loading history:', err); toast.error('Error al cargar historial'); }
  };
  ...
  return (
    <Box>
      ...
      {initError && <Alert severity="warning" sx={{ mb: 2 }} onClose={() => setInitError(null)}>{initError}</Alert>}
