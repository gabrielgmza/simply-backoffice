...
export default function Aria() {
  ...
  const loadSessions = async () => {
    try { const res = await api.get('/api/backoffice/aria/sessions'); setSessions(res.data.data || []); } 
    catch (err) { console.error(err); }
  };

  const loadSession = async (sessionId: string) => {
    try {
      const res = await api.get(`/api/backoffice/aria/sessions/${sessionId}`);
  ...
  const sendMessage = async () => {
    ...
    setError(null);
    try {
      const res = await api.post(`/api/backoffice/aria/sessions/${currentSession}/chat`, { message: userMessage });
  ...
  const rollbackDecision = async (decisionId: string) => {
    ...
    if (!reason) return;
    try { await api.post(`/api/backoffice/aria/decisions/${decisionId}/rollback`, { reason }); loadSession(currentSession!); }
    catch (err: any) { setError(err.response?.data?.error || 'Error'); }
  };
