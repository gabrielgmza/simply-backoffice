...
export default function BCRA() {
  ...
  const loadCotizacion = async () => {
    ...
    try {
      const [cotRes, histRes] = await Promise.all([
        api.get('/api/bcra/usd'),
        api.get('/api/bcra/usd/historico?dias=30')
      ]);
  ...
  const buscarDeudor = async () => {
    ...
    try {
      const res = await api.get(`/api/bcra/riesgo/${cuit.replace(/[-\s]/g, '')}`);
      setEvaluacion(res.data.data);
    } catch (err: any) { toast.error(err.response?.data?.error || 'Error en consulta'); }
