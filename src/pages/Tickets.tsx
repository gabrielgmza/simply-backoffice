...
export default function Tickets() {
  ...
  const loadTickets = async () => {
    ...
    try {
      ...
      const res = await api.get(`/api/backoffice/tickets?${params}`);
  ...
  const loadStats = async () => {
    try { const res = await api.get('/api/backoffice/tickets/stats'); setStats(res.data.data); } catch (err) { console.error(err); }
  };

  const openTicket = async (ticket: Ticket) => {
    setSelectedTicket(ticket);
    setDialogOpen(true);
    try { const res = await api.get(`/api/backoffice/tickets/${ticket.id}`); setMessages(res.data.data.messages || []); } catch (err) { console.error(err); }
  };

  const sendMessage = async () => {
    if (!newMessage.trim() || !selectedTicket) return;
    try {
      await api.post(`/api/backoffice/tickets/${selectedTicket.id}/messages`, { content: newMessage, isInternal: tab === 1 });
      setNewMessage('');
      const res = await api.get(`/api/backoffice/tickets/${selectedTicket.id}`);
  ...
  const letAriaRespond = async () => {
    ...
    setAriaLoading(true);
    try {
      const res = await api.post(`/api/backoffice/tickets/${selectedTicket.id}/aria-respond`);
  ...
  const closeTicket = async () => {
    ...
    try { await api.post(`/api/backoffice/tickets/${selectedTicket.id}/close`, { resolution }); setDialogOpen(false); loadTickets(); }
    catch (err: any) { setError(err.response?.data?.error || 'Error'); }
  };
