...
export default function Treasury() {
  ...
  const loadData = async () => {
    try {
      const [accountsRes, summaryRes, cashflowRes] = await Promise.all([
        api.get('/backoffice/treasury/accounts'),
        api.get('/backoffice/treasury/summary'),
        api.get('/backoffice/treasury/cashflow'),
      ]);
  ...
  const handleDeposit = async () => {
    try {
      await api.post('/backoffice/treasury/deposit', {
  ...
  const handleWithdraw = async () => {
    try {
      await api.post('/backoffice/treasury/withdraw', {
        accountId: formAccountId,
        amount: parseFloat(formAmount),
        description: formDescription,
      });
  ...
  const handleTransfer = async () => {
    try {
      await api.post('/backoffice/treasury/transfer', {
